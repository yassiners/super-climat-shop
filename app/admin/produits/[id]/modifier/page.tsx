...
export default function ModifierProduitPage({ params }: { params: { id: string } }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res  = await fetch(`/api/admin/produits/${params.id}`, { method: 'PUT', body: formData });
      const data = await res.json();
      if (res.ok) {
        router.push('/admin/produits?msg=updated');
      } else {
        setErrors([data.error ?? 'Erreur lors de la modification.']);
      }
    } catch {
      ...
      <div className="admin-main">
        <div className="admin-topbar">
          <h1 className="admin-topbar-title">
            <i className="fa-solid fa-pen" style={{ color: 'var(--primary)', marginRight: 8 }}></i>
            Modifier le Produit
          </h1>
        ...
        <div className="admin-content">
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data" id="editProductForm">
            <div className="admin-form-card">
              ...
              <div className="form-group">
                ...
                <input type="text" className="form-control" id="p-nom" name="nom"
                  required defaultValue={produit.nom} />
              </div>

              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="p-cat">Catégorie <span className="req">*</span></label>
                  <select className="form-control" id="p-cat" name="categorie" required
                ...
                <div className="form-group">
                  ...
                  <select className="form-control" id="p-subcat" name="sous_categorie"
                    defaultValue={produit.sous_categorie ?? ''}>
                    <option value="">— Choisir —</option>
                    {selectedCat && filteredSousCats.map(sc => (
              ...
              <div className="form-group">
                ...
                <input type="number" className="form-control" id="p-prix" name="prix"
                  min="0" step="0.01" defaultValue={produit.prix}
                  style={{ maxWidth: 220 }} />
              </div>
              ...
              <div className="form-group">
                ...
                <textarea className="form-control" id="p-desc" name="description" rows={5}
                  defaultValue={produit.description ?? ''}></textarea>
              </div>

              <div className="form-group">
                <label>Image du produit</label>
                {produit.image && !previewUrl && (
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: '.8rem', color: 'var(--text-muted)', marginBottom: 8 }}>Image actuelle :</p>
                    <Image src={produit.image} alt="Image actuelle" width={180} height={120}
                      style={{ objectFit: 'cover', borderRadius: 8, boxShadow: 'var(--shadow-sm)' }} />
                  </div>
                )}
                ...
                <div className="upload-zone" id="uploadZone"
                  ...
                  <p>{produit.image ? 'Cliquez pour remplacer l\'image' : 'Cliquez ou glissez une nouvelle image'} (JPG, PNG, WEBP — max 5 Mo)</p>
                </div>
                {previewUrl && (
                  <img id="uploadPreview" className="upload-preview" src={previewUrl}
                    alt="Aperçu nouvelle image" style={{ display: 'block', marginTop: 12, maxWidth: 200 }} />
                )}
              </div>

              <div style={{ display: 'flex', gap: 12, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
                <button type="submit" className="btn btn-primary" id="edit-submit-btn" disabled={loading}>
                  <i className="fa-solid fa-floppy-disk"></i>{' '}
                  {loading ? 'Enregistrement...' : 'Enregistrer les modifications'}
                </button>
                <Link href="/admin/produits" className="btn btn-outline" id="edit-cancel-btn">Annuler</Link>
              </div>
            </div>
          </form>
        </div>
